...
- `api/lib/eodhd.js` is the server-only EODHD adapter. It reads `EODHD_API_TOKEN`, keeps the token out of browser code, normalizes the curated universe, and supports bounded discovery through EODHD `GET /api/exchange-symbol-list/{exchange}` plus `GET /api/search/{query}`. The app currently imports up to 100 records per page from SIX (`SW`), XETRA, LSE and NASDAQ, deduplicates by ISIN or exchange symbol, then hydrates the selected records with daily EOD history.
- `/api/instruments` accepts `import=0|1`, `page`, `limit` (maximum 100), `type=etf|stock`, `market=Switzerland|Europe|Worldwide`, `chf=1`, `hedged=1`, `exchanges=SW,XETRA,LSE,NASDAQ`, and optional `q`. Discovery warnings are returned in the `warnings` array and do not discard usable curated or imported instruments. EODHD exchange lists are provider-plan dependent and are not a complete global security master; coverage, symbol availability, history access and the 500-record discovery safety cap limit results. The dashboard therefore labels the count as loaded/imported data, not a complete market universe.
